...
function logArrayElements(arr) {
  ...
  if (!Array.isArray(arr)) {
    ...
    for (let element of arr) {
      console.log(`Element ${arr.indexOf(element)}: ${element}`);
    }
  }
}
...
function compareArrays(arr1, arr2) {
  ...
  let sortArray1 = [...arr1].sort();
  let sortArray2 = [...arr2].sort();

  let toJson1 = JSON.stringify(sortArray1);
  let toJson2 = JSON.stringify(sortArray2);

  if (toJson1 === toJson2) {
    console.log("Arr is same");
  } else {
...
function buildMatrix(size) {
  ...
  if (typeof size !== "number") {
    ...
    for (let i = 0; i < size; i++) {
      ...
      ourMatrix.push(matrixRow);
    }
    console.table(ourMatrix);
    return ourMatrix;
  }
}
...
function countVowelsAndConsonants(word) {
  ...
  console.log(`Кількість приголосних: ${consonants}`);
}
